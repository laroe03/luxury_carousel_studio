...
// ─── Clients (lazy-initialised so missing keys don't crash the other provider) ──
...
async function generateWithOpenAI(
  ...
  if (!output) {
    // Retry without json_object mode as fallback
...
export async function generateCarousel(
  ...
  return generateWithAnthropic(prompt, signal);
}
